...
fun LoginScreen(
    ...
    loginScreeViewModel: LoginScreenViewModel = viewModel()
) {
    Surface(
        ...
        val username by loginScreeViewModel.username.observeAsState(initial = "")
        val password by loginScreeViewModel.password.observeAsState(initial = "")
        val imageUri by loginScreeViewModel.imageUri.observeAsState(initial = null)
        val resultLauncher = rememberLauncherForActivityResult(
            contract = ActivityResultContracts.StartActivityForResult(),
            onResult = { result ->
                if (result.resultCode == Activity.RESULT_OK) {
                    val uri = result.data?.data
                    loginScreeViewModel.updateImageUri(uri)
                }
            }
        )
        ...
        LoginProfileContent(
            ...
            username = username,
            password = password,
            onUsernameChanged = { newValue -> loginScreeViewModel.updateUsername(newValue) },
            onPasswordChanged = { newValue -> loginScreeViewModel.updatePassword(newValue) },
            onSignInButtonClicked = {
                isLoading = false
                loginScreeViewModel.fetchProfile()
            },
            onSignUpButtonClicked = {
                loginScreeViewModel.sendDataToFireBase(context)
                loginScreeViewModel.sendDataToWear(context, dataClient)
                val userData = LoginInfo(
                    loginScreeViewModel.username.value ?: "",
                    loginScreeViewModel.imageUri.value,
                    loginScreeViewModel.key
                )
        ...
        val uploadSuccess by loginScreeViewModel.uploadSuccess
        ...
        val profilePresent by loginScreeViewModel.profilePresent.observeAsState()
        profilePresent?.let { present ->
            if (present) {
                loginScreeViewModel.loadUserImageUri(context)
            } else {
                isLoading = false
                Toast.makeText(context, "Incorrect username/password",
                    Toast.LENGTH_LONG).show()
                loginScreeViewModel.resetProfilePresent()
            }
        }

        val userImageLoadingFinished by loginScreeViewModel.userImageLoadingFinished
            .observeAsState()
        userImageLoadingFinished?.let {
            ...
            loginScreeViewModel.sendDataToWear(context, dataClient, true)
...
fun LoginProfileContent(
    ...
    modifier: Modifier = Modifier,
    loginProfileViewModel: LoginScreenViewModel = viewModel()
) {
    val context = LocalContext.current
